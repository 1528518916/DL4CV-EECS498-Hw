...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for out_channel in stem_channels:
            stem_cls.extend([
                nn.Conv2d(cur_channel, out_channels=out_channel, kernel_size=3, stride=1, padding=1),
                nn.ReLU()
            ])
            stem_box.extend([
                nn.Conv2d(cur_channel, out_channels=out_channel, kernel_size=3, stride=1, padding=1),
        ...
        for layers in [stem_cls, stem_box]:
            for layer in layers:
        ...
        self.pred_cls = nn.Conv2d(cur_channel, num_classes, kernel_size=3, padding=1, stride=1)  # Class prediction conv
        self.pred_box = nn.Conv2d(cur_channel, 4, kernel_size=3, padding=1, stride=1)  # Box regression conv
        self.pred_ctr = nn.Conv2d(cur_channel, 1, kernel_size=3, padding=1, stride=1)  # Centerness conv
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        # Replace "pass" statement with your code
        for level_name, feats_map in feats_per_fpn_level.items():
            batch_size = feats_map.shape[0] # (N, C, H, W)
            cls_feats = self.stem_cls(feats_map)
            box_feats = self.stem_box(feats_map)
            
            cls_pred = self.pred_cls(cls_feats)
            box_pred = self.pred_box(box_feats)
            ctr_pred = self.pred_ctr(box_feats)

            H, W = cls_pred.shape[2:]

            class_logits[level_name] = cls_pred.view(batch_size, -1, H * W).transpose(1, 2)
            boxreg_deltas[level_name] = box_pred.view(batch_size, 4, H * W).transpose(1, 2)
            centerness_logits[level_name] = ctr_pred.view(batch_size, 1, H * W).transpose(1, 2)
...
def fcos_match_locations_to_gt(
    ...
    for level_name, centers in locations_per_fpn_level.items():
        ...
        # centers (H*W, 2)
        # gt_boxes (M, 5)
        x, y = centers.unsqueeze(dim=2).unbind(dim=1) # x, y (H*W, 1)
        x0, y0, x1, y1 = gt_boxes[:, :4].unsqueeze(dim=0).unbind(dim=2) # x0, y0, x1, y1 (1, M)
        pairwise_dist = torch.stack([x - x0, y - y0, x1 - x, y1 - y], dim=2)
        # x - x0        (H*W, M)
        # pairwise_dist (H*W, M, 4)
        # print(f'Gt_box shape : {gt_boxes.shape}')
        ...
        # print(f'pairwise_dist shape : {pairwise_dist.shape}')
        ...
        # print(f'gt_areas shape : {gt_areas.shape}')
        ...
        # print(f'matched_idx : {matched_idxs.shape}')

        # Anchors with label 0 are treated as background.
        matched_boxes_this_level = gt_boxes[matched_idxs.clip(min=0)]
        # print(f'matched_idxs.clip(min = 0) : {matched_idxs.clip(min=0).shape}')
        matched_boxes_this_level[matched_idxs < 0, :] = -1
        # print(f'matched_boxes_this_level: {matched_boxes_this_level.shape}')
...
def fcos_get_deltas_from_locations(
    ...
    for i in range(N):
        # Check if it's a background box
        if torch.all(gt_boxes[i][:4] == -1):
            deltas[i] = torch.tensor([-1, -1, -1, -1], 
                                   dtype=locations.dtype, 
                                   device=locations.device)
        else:
            # Calculate LTRB deltas
            l = locations[i][0] - gt_boxes[i][0]  # left
            t = locations[i][1] - gt_boxes[i][1]  # top
            r = gt_boxes[i][2] - locations[i][0]  # right
            b = gt_boxes[i][3] - locations[i][1]  # bottom
            
            # Normalize by stride
            deltas[i] = torch.tensor([l, t, r, b], 
                                   dtype=locations.dtype, 
                                   device=locations.device) / stride
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    # # Replace "pass" statement with your code
    # N = deltas.shape[0]
    # centerness = torch.full((N,), -1, dtype=deltas.dtype, device=deltas.device)
    # for i in range(N):
    #     if torch.all(deltas[i] == -1):
    #         continue
    #     l, t, r, b = deltas[i]
    #     centerness[i] = ((torch.min(l, r) * torch.min(t, b)) / (torch.max(l, r) * torch.max(t, b))).sqrt()
    
    mask = ~torch.all(deltas == -1, dim=1)
    centerness = torch.full((deltas.shape[0],), -1, dtype=deltas.dtype, device=deltas.device)
    
    if mask.any():
        valid_deltas = deltas[mask]
        l, t, r, b = valid_deltas.unbind(dim=1)
        ...
        centerness[mask] = valid_centerness
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        shape_per_fpn_level = {
            level_name: fpn.shape for level_name, fpn in fpn_feats.items()
        }
        locations_per_fpn_level = get_fpn_location_coords(shape_per_fpn_level, fpn_strides)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        for i in range(batch_size):
            matched_gt_boxes.append(fcos_match_locations_to_gt(locations_per_fpn_level, fpn_strides, gt_boxes[i]))
        ...
        for i in range(batch_size):
            cur = {}
            for level_name, level_matched_boxes in matched_gt_boxes[i].items():
                cur[level_name] = fcos_get_deltas_from_locations(
                    locations_per_fpn_level[level_name],
                    level_matched_boxes,
                    fpn_strides[level_name]
                )
            matched_gt_deltas.append(cur)
        ...
        # Replace "pass" statement with your code
        B, L = matched_gt_boxes.shape[0], matched_gt_boxes.shape[1]
        gt_classes = F.one_hot(matched_gt_boxes[:,:,4] + 1, num_classes=self.num_classes + 1)
        ...
        pred_ctr_logits = pred_ctr_logits.reshape(-1)
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 3:
            # Replace "pass" statement with your code
            level_pred_boxes = fcos_apply_deltas_to_locations(level_deltas, level_locations, fpn_strides[level_name])
            level_pred_boxes = level_pred_boxes[keep]

            not_bg = (level_deltas[keep].sum(dim=1) != -4)
            ...
            level_pred_boxes[:, 1].clamp_(max=images.shape[3])
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
