...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores -= scores.max(dim=1, keepdim=True)[0] # Stability
    exp_scores = torch.exp(scores)
    probs = exp_scores / exp_scores.sum(dim=1, keepdim=True) # For probabilities
...
def nn_predict(
    ...
    y_pred = torch.argmax(scores, dim=1)[0]
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
