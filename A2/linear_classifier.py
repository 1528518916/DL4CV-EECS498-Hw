...
def svm_loss_vectorized(
    ...
    # Replace "pass" statement with your code
    score = X.mm(W)
    correct_class_score = score[torch.arange(X.shape[0]), y].view(X.shape[0], -1)
    ...
    # Replace "pass" statement with your code
    margin[margin > 0] = 1
    margin[torch.arange(X.shape[0]), y] = - torch.sum(margin, dim=1)
    dW = X.t().mm(margin) / X.shape[0] + 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def svm_get_search_params():
    ...
    learning_rates = [0.01, 0.015, 0.005]
    regularization_strengths = [0.001, 0.005, 0.010]
...
Returns:
    ...
    train_acc = (cls.predict(X_train) == y_train).sum() / y_train.shape[0]
    val_acc = (cls.predict(X_val) == y_val).sum() / y_val.shape[0]
...
def softmax_loss_naive(
    ...
    for i in range(N):
        ...
        denom = torch.sum(score)
        loss += - torch.log(score[y[i]] / denom)
        dW[:, y[i]] -= X[i]
        dW += X[i].view(-1, 1) * (score / denom).view(1, -1)
...
def softmax_loss_vectorized(
    ...
    denoms = torch.sum(scores, dim=1, keepdim=True) # (N, 1)
    soft_scores = scores / denoms # (N, C)
    soft_scores[torch.arange(N), y] -= 1

    loss = -torch.sum(torch.log(soft_scores[torch.arange(N), y]))
    dW += X.t().mm(soft_scores)
