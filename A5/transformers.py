...
def generate_token_dict(vocab):
    ...
    token_dict = {
        voc: idx for idx, voc in enumerate(vocab)
    }
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def scaled_dot_product_two_loop_single(
    ...
    for k in range(K):
        simi = torch.zeros((K,), dtype=query.dtype, device=query.device)
        for l in range(K):
            simi[l] = (query[k] * key[l]).sum()
        simi /= (M ** 0.5)
        simi = torch.softmax(simi, dim=0)
        for l in range(K):
            out[k] += simi[l] * value[l]
...
def scaled_dot_product_two_loop_batch(
    ...
    # vertorized Batched ver
    # simi = torch.bmm(query, key.permute(0, 2, 1))
    # simi /= scale
    # simi = torch.softmax(simi, dim=-1) # (N, K, K) containing weights
    # # simi * value (N, K, K) * (N, K, M)
    # out  = torch.bmm(simi, value)

    for k in range(K):
        simi = torch.zeros((N, K), dtype=out.dtype, device=out.device)
        for l in range(K):
            simi[:, l] = (query[:, k] * key[:, l]).sum(dim=1)
        simi /= scale
        simi = torch.softmax(simi, dim=1)
        for l in range(K):
            out[:, k] += (simi[:, l].reshape(-1, 1) * value[:, l]).squeeze()
...
def scaled_dot_product_no_loop_batch(
    ...
    simi /= scale
    if flag == True:
        simi.masked_fill_(mask, torch.tensor(-1e9))
    simi = torch.softmax(simi, dim=-1) # (N, K, K) containing weights
    # simi * value (N, K, K) * (N, K, M)
    y    = torch.bmm(simi, value)
    weights_softmax = simi
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        nn.init.uniform_(self.k.weight, -cq, cq)
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        t_query = self.q(query)
        t_key   = self.k(key)
        t_value = self.v(value)
        y, self.weights_softmax = scaled_dot_product_no_loop_batch(t_query, t_key, t_value, mask)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        for _ in range(num_heads):
            self.attn_layers.append(SelfAttention(dim_in=dim_in, dim_q=dim_out, dim_v=dim_out))
        
        self.fc = nn.Linear(num_heads * dim_out, dim_in)
        c   = (6 / (num_heads * dim_out + dim_in)) ** 0.5
        nn.init.uniform_(self.fc.weight, -c, c)
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # Compute mean and variance over the embedding dimension M
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.multihead_attn = MultiHeadAttention(num_heads=num_heads, dim_in=emb_dim, dim_out=(emb_dim // num_heads))
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N, K = seq.shape
    mask = torch.full((N, K, K), False, dtype=torch.bool, device=seq.device)
    for k in range(K):
        mask[:, k, k+1:] = True
    ##############################################################################
    #               END OF YOUR CODE                                             #
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    for i in range(K):
        for j in range(M):
            if j % 2 == 0:
                y[0, i, j] = torch.sin(torch.tensor(i / ((1e4) ** (j // M))))
            else:
                y[0, i, j] = torch.cos(torch.tensor(i / ((1e4) ** ((j-1) // M))))
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        print(ques_b.shape, ques_pos.shape, ans_b.shape, ans_pos.shape)

        N, K = ans_b.shape
        enc_out = self.encoder(q_emb_inp)
        mask = get_subsequent_mask(ans_b[:,:-1])
        dec_out = self.decoder(a_emb_inp, enc_out, mask)
        Np, Kp = dec_out.shape[0], dec_out.shape[1]
        dec_out = dec_out.reshape(Np * Kp, -1)
        print(dec_out.shape)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
