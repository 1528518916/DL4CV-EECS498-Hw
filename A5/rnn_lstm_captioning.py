...
def rnn_step_forward(x: torch.Tensor, prev_h: torch.Tensor, Wx: torch.Tensor, Wh: torch.Tensor, b: torch.Tensor):
    ...
    ##########################################################################
    # DONE: Implement a single forward step for the vanilla RNN. Store next
    # hidden state and any values you need for the backward pass in the next_h
    # and cache variables respectively.
    ##########################################################################
    # Replace "pass" statement with your code
    next_h = torch.tanh(x.mm(Wx) + prev_h.mm(Wh) + b)
    cache = (x, prev_h, Wx, Wh, next_h) # TODO: Is next_h needed?
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return next_h, cache
...
def rnn_forward(x: torch.Tensor, h0: torch.Tensor, Wx: torch.Tensor, Wh: torch.Tensor, b: torch.Tensor):
    ...
    for t in range(T):
        xt = x[:, t, :]
        cur_h, cur_cache = rnn_step_forward(xt, prev_h, Wx, Wh, b)
        h[:, t, :] = cur_h
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dh_cur = dh[:, t, :] + dprev_h
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dh_cur, cache[t])
...
def temporal_softmax_loss(x: torch.Tensor, y: torch.Tensor, ignore_index=None):
    """
    ...
    ##########################################################################
    # Done: Implement the temporal softmax loss function.
    #
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    loss = F.cross_entropy(input=x.permute(0, 2, 1), target=y, reduction='sum', ignore_index=ignore_index) / x.shape[0]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        # img_encoder    : image       (N, C, H, W)      -> grid_feat    (N, C', H', W')
        # feat_img_input : grid_feat   (N, C', H', W')   -> img_feat     (N, D)
        # feats_hidden   : img_feat    (N, D)            -> hidden_ini   (N, H)

        # embedder       : word idx    (N, T)            -> embedding    (N, T, W)
        # rnn            : embedding   (N, T, W), (N, H) -> hidden       (N, T, H)
        # rnn.step       : embedding   (N, W), (N, H)    -> hidden       (N, H)
        # hidden_out     : hidden      (N, T, H)         -> scores       (N, T, V)

        # N : Batch size
        # C : Channel size, default 3
        # T : Caption length
        # D : input_dim, img_feature dimension == C'
        # W : wordvec_dim, word_vector dimension

        self.img_encoder = ImageEncoder()
        if self.cell_type == 'attn':
            self.imgfeats_inputfeats = nn.Conv2d(input_dim, hidden_dim, kernel_size=1, stride=1, padding=0)
        else:
            self.imgfeats_inputfeats = nn.Sequential( # img
            nn.AdaptiveAvgPool2d((1, 1)), # (N, C, 1, 1)
            Flatten(),                    # (N, C) # Wtf img_channel == input_dim QAQ
        )
        if self.cell_type == 'attn':
            self.feats_hidden = nn.Identity()
        else:
            self.feats_hidden = nn.Linear(in_features=input_dim, out_features=hidden_dim)
       
        self.embedder    = WordEmbedding(vocab_size, wordvec_dim)
        if self.cell_type   == 'rnn':
            ...
            self.attn  = AttentionLSTM(input_dim=wordvec_dim, hidden_dim=hidden_dim)
        self.hidden_out   = nn.Linear(in_features=hidden_dim, out_features=vocab_size)
    ...
    def forward(self, images, captions):
        ...
        # 1. img -> img_features
        # (N, C, H, W) -> (N, out_channels, H / 32, W / 32)
        feats = self.img_encoder(images)

        # 2. img_features -> flatten_features
        # (N, out_channels, H / 32, W / 32) -> (N, input_dim)
        feats = self.imgfeats_inputfeats(feats)

        # 3. flatten_features -> initial_hidden_states
        # (N, input_dim) -> (N, hidden_dim) or (N, H) 
        init_hidden_states = self.feats_hidden(feats)

        # 4. captions_in -> input_vectors
        # (N, T) -> (N, T, W)
        captions_feats = self.embedder(captions_in)

        # 5. initial_hidden_states + input_vectors -> all hidden states
        # (N, T, V) + (N, H) -> (N, T, H)
        if self.cell_type == 'rnn':
            hidden_states = self.rnn(captions_feats, init_hidden_states)
        elif self.cell_type == 'lstm':
            hidden_states = self.lstm(captions_feats, init_hidden_states)
        else:
            hidden_states = self.attn(captions_feats, init_hidden_states)

        # 6. hidden_states -> scores on each word
        # (N, T, H) -> (N, T, V)
        scores = self.hidden_out(hidden_states)

        # 7. Compute loss using temporal_sfm_loss
        # (N, T, V), (N, T) -> scalar
    ...
    def sample(self, images, max_length=15):
        ...
        # Replace "pass" statement with your code
        N = images.shape[0]
        grid_feat  = self.img_encoder(images)
        img_feat   = self.imgfeats_inputfeats(grid_feat)
        cur_hidden = self.feats_hidden(img_feat)

        if self.cell_type == 'lstm':
            cur_cell = torch.zeros_like(cur_hidden)
        elif self.cell_type == 'attn':
            A = cur_hidden
            cur_hidden = A.mean(dim=(2, 3))
            cur_cell = cur_hidden
        
        # if self.cell_type 
        
        cur_word_idx = torch.full((N,), self._start)
        for t in range(max_length):
            cur_word_embed = self.embedder(cur_word_idx)
            if self.cell_type   == 'rnn':
                cur_hidden  = self.rnn.step_forward(cur_word_embed, cur_hidden)
            elif self.cell_type == 'lstm':
                cur_hidden, cur_cell = self.lstm.step_forward(cur_word_embed, cur_hidden, cur_cell)
            else:
                attn, attn_weights = dot_product_attention(cur_hidden, A)
                cur_hidden, cur_cell = self.attn.step_forward(cur_word_embed, cur_hidden, cur_cell, attn)
                attn_weights_all[:, t] = attn_weights
            scores         = self.hidden_out(cur_hidden)
            cur_word_idx   = torch.argmax(scores, dim=1)
            captions[:, t] = cur_word_idx
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        N, H = prev_h.shape
        activations = x @ self.Wx + prev_h @ self.Wh + self.b # (N, 4H)

        i = torch.sigmoid(activations[:,0:H])
        f = torch.sigmoid(activations[:,H:2*H])
        o = torch.sigmoid(activations[:,2*H:3*H])
        g = torch.tanh(activations[:,3*H:])
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        cur_h, cur_c = h0, c0
        for t in range(T):
            cur_h, cur_c = self.step_forward(x[:, t], cur_h, cur_c)
            hn[:, t] = cur_h
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    prev_h_expanded = prev_h.reshape(N, H, 1, 1)
    scaled_dots     = (prev_h_expanded * A).sum(dim=1, keepdim=True) / (H ** 0.5)
    flatten_dots    = scaled_dots.reshape(N, -1)
    flatten_weights = torch.softmax(flatten_dots, dim=1)
    attn_weights    = flatten_weights.reshape(N, D_a, D_a)
    flatten_A       = A.reshape(N, H, -1)
    expanded_weights = flatten_weights.unsqueeze(1)
    attn = (flatten_A * expanded_weights).sum(dim=2)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        N, H = prev_c.shape
        activations = x @ self.Wx + prev_h @ self.Wh + attn @ self.Wattn + self.b # (N, 4H)
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        cur_h, cur_c = h0, c0
        for t in range(T):
            attn, attn_weights = dot_product_attention(cur_h, A)
            cur_h, cur_c = self.step_forward(x[:, t], cur_h, cur_c, attn)
            hn[:, t] = cur_h
