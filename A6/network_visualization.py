...
def compute_saliency_maps(X, y, model):
    ...
    scores = model(X) # (N, C)
    loss = scores[torch.arange(scores.shape[0]), y].sum()
    loss.backward()
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    for i in range(max_iter):
        ...
        if torch.argmax(scores, dim=1).flatten().item() == target_y:
            break
        with torch.no_grad():
            dX = learning_rate * X_adv.grad / torch.norm(X_adv.grad)
            X_adv += X_adv.grad * learning_rate / torch.norm(X_adv.grad)
        X_adv.grad.zero_()
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    reg_loss = l2_reg * torch.norm(img)
    loss     = target_score - reg_loss
