...
def gram_matrix(features, normalize=True):
    ...
    features_flatten = features.flatten(2, -1)
    gram = torch.bmm(features_flatten, features_flatten.permute(0, 2, 1))
    assert(list(gram.shape) == [N, C, C])
    if normalize:
       gram /= (H * W * C)
    ############################################################################
    #                               END OF YOUR CODE                           #
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for idx, tar, weights in zip(style_layers, style_targets, style_weights):
      loss += ((gram_matrix(feats[idx]) - tar) ** 2).sum() * weights
...
def guided_gram_matrix(features, masks, normalize=True):
  ...
  features = features * masks.unsqueeze(dim=2)
  features_flatten = features.flatten(3, -1)
  out = features_flatten @ features_flatten.permute(0, 1, 3, 2)
  if normalize:
     out /= H * W * C
  return out
  ##############################################################################
...
def guided_style_loss(feats, style_layers, style_targets, style_weights, content_masks):
    ...
    # Replace "pass" statement with your code
    loss = 0
    for id, tar, wei in zip(style_layers, style_targets, style_weights):
      loss += wei * ((guided_gram_matrix(feats[id], content_masks[id]) - tar) ** 2).sum()
    return loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
