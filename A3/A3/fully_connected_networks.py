...
class Linear(object):
    ...
    @staticmethod
    def backward(dout: torch.Tensor, cache: torch.Tensor):
        """
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        assert(self.params['W1'].shape == torch.Size([input_dim, hidden_dim]))
        assert(self.params['W2'].shape == torch.Size([hidden_dim, num_classes]))
        assert(self.params['b1'].shape == torch.Size([hidden_dim, ]))
        assert(self.params['b2'].shape == torch.Size([num_classes, ]))
        ###############################################################
        #                            END OF YOUR CODE                 #
        ###############################################################
    ...
    def loss(self, X, y=None):
        ...
        lrl_out, lrl_cache = Linear.forward(lr_out, self.params['W2'], self.params['b2'])
        # lrl_out: (N, C)
        # scores = torch.softmax(lrl_out, dim=1) # TODO
        scores = lrl_out
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        # dx, dw, db
        dout, grads['W2'], grads['b2'] = Linear.backward(df, lrl_cache)
        _, grads['W1'], grads['b1'] = Linear_ReLU.backward(dout, lr_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims: list, input_dim=3*32*32, num_classes=10,
        ...
        #######################################################################
        # Replace "pass" statement with your code
        curdim, nedim = None, input_dim
        hidden_dims.append(num_classes)
        for i in range(self.num_layers):
            curdim, nedim = nedim, hidden_dims[i]
            self.params[f'W{i + 1}'] = weight_scale * torch.randn((curdim, nedim), dtype=dtype, device=device)
            self.params[f'b{i + 1}'] = torch.zeros((nedim,), dtype=dtype, device=device)
        hidden_dims.pop()
    ...
    def loss(self, X: torch.Tensor, y: torch.Tensor=None):
        """
        ...
        for i in range(1, self.num_layers + 1):
          if i != self.num_layers:
            cur_x, cache_dict[f'cache_{i}'] = Linear_ReLU.forward(cur_x, self.params[f'W{i}'], self.params[f'b{i}'])
            if self.use_dropout:
              cur_x, cache_dict[f'mask_{i}'] = Dropout.forward(cur_x, self.dropout_param)
          else:
            cur_x, cache_dict[f'cache_{i}'] = Linear.forward(cur_x, self.params[f'W{i}'], self.params[f'b{i}'])
        scores = cur_x
        ...
        for i in range(self.num_layers, 0, -1):
            if i == self.num_layers:
              dout, grads[f'W{i}'], grads[f'b{i}'] = Linear.backward(dout, cache_dict[f'cache_{i}'])
            else:
              if self.use_dropout:
                dout = Dropout.backward(dout, cache_dict[f'mask_{i}'])
              dout, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dout, cache_dict[f'cache_{i}'])
...
def get_three_layer_network_params():
    ...
    ###############################################################
    weight_scale = 1e1   # Experiment with this!
...
def rmsprop(w: torch.Tensor, dw: torch.Tensor, config=None):
    """
...
class Dropout(object):
    ...
    @staticmethod
    def forward(x: torch.Tensor, dropout_param):
        """
