...
class Conv(object):
    ...
    @staticmethod
    def forward(x: torch.Tensor, w: torch.Tensor, b: torch.Tensor, conv_param):
        """
        ...
        out = torch.zeros((N, F, Hp, Wp), dtype=x.dtype, device=x.device)
        # x   (N, C, H , W )
        # x'  (N, C, HH, WW)
        # w   (F, C, HH, WW)
        # out (N, F, Hp, Wp)
        for i in range(Hp):
          for j in range(Wp):
            ...
            out[:, :, i, j] = torch.sum(x_slice.unsqueeze(1) * w.unsqueeze(0), dim=(2, 3, 4))
        # for n in range(N):
        #   for f in range(F):
        #     for i in range(0, Hp):
        #       for j in range(0, Wp):
        #         out[n, f, i, j] = torch.sum(x_padded[n, :, (i*stride):(i*stride+HH), (j*stride):(j*stride+WW)] * w[f])
        out += b.reshape(F, 1, 1)
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
    ...
    def backward(dout: torch.Tensor, cache):
        ...
        for i in range(N):
          for j in range(F):
            for m in range(H2):
              for n in range(W2):
                dx_padded[i, :, m*stride:m*stride+HH, n*stride:n*stride+WW] += w[j] * dout[i,j,m,n]
                dw[j] += x_padded[i, :, m*stride:m*stride+HH, n*stride:n*stride+WW] * dout[i,j,m,n]
...
class MaxPool(object):
    ...
    @staticmethod
    def forward(x: torch.Tensor, pool_param):
        """
        ...
        for n in range(N):
          for c in range(C):
            for i in range(Hp):
              for j in range(Wp):
                out[n, c, i, j] = torch.max(x[n, c, i*stride:i*stride+ph, j*stride:j*stride+pw])
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
          ...
          for c in range(C):
            for i in range(Hp):
              for j in range(Wp):
                ...
                h_idx = max_idx // pw + i * stride
                w_idx = max_idx % pw + j * stride
...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # Replace "pass" statement with your code
        cache_dict = {}
        cur_x, cache_dict['crp'] = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
        cur_x, cache_dict['lr'] = Linear_ReLU.forward(cur_x, W2, b2)
        cur_x, cache_dict['l'] = Linear.forward(cur_x, W3, b3)
        scores = cur_x
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dx, grads['W3'], grads['b3'] = Linear.backward(dx, cache_dict['l'])
        dx, grads['W2'], grads['b2'] = Linear_ReLU.backward(dx, cache_dict['lr'])
        dx, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dx, cache_dict['crp'])
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        #####################################################################
        # Replace "pass" statement with your code
        use_kai = weight_scale == 'kaiming'
        C, H, W = input_dims
        L = len(num_filters)
        C_cur, H_cur, W_cur = C, H, W
        kernel_size = 3
        for i in range(L):
          if use_kai:
            self.params[f'W{i}'] = kaiming_initializer(C_cur, num_filters[i], kernel_size, dtype=dtype, device=device)
          else:
            self.params[f'W{i}'] = weight_scale * torch.randn((num_filters[i], C_cur, kernel_size, kernel_size), dtype=dtype, device=device)
          self.params[f'b{i}'] = torch.zeros((num_filters[i],), dtype=dtype, device=device)
          # Batch Normalization
          if self.batchnorm:
            self.params[f'gamma{i}'] = torch.ones((num_filters[i],), dtype=dtype, device=device)
            self.params[f'beta{i}'] = torch.zeros((num_filters[i],), dtype=dtype, device=device)

          C_cur = num_filters[i]
          if i in max_pools:
            H_cur, W_cur = H_cur // 2, W_cur // 2
        if use_kai:
          self.params[f'W{L}'] = kaiming_initializer(num_filters[L-1]*H_cur*W_cur, num_classes, K=None, dtype=dtype, device=device)
        else:
          self.params[f'W{L}'] = weight_scale * torch.randn((num_filters[L-1] * H_cur * W_cur, num_classes), dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(L):
          if i in self.max_pools:
            if self.batchnorm:
              cur_x, cache_dict[f'layer_{i}'] = Conv_BatchNorm_ReLU_Pool.forward(cur_x, self.params[f'W{i}'], self.params[f'b{i}'], self.params[f'gamma{i}'], self.params[f'beta{i}'], conv_param=conv_param, pool_param=pool_param, bn_param=self.bn_params[i])
            else:
              cur_x, cache_dict[f'layer_{i}'] = Conv_ReLU_Pool.forward(cur_x, self.params[f'W{i}'], self.params[f'b{i}'], conv_param=conv_param, pool_param=pool_param)
          else:
            if self.batchnorm:
              cur_x, cache_dict[f'layer_{i}'] = Conv_BatchNorm_ReLU.forward(cur_x, self.params[f'W{i}'], self.params[f'b{i}'], self.params[f'gamma{i}'], self.params[f'beta{i}'], conv_param=conv_param, bn_param=self.bn_params[i])
            else:
              cur_x, cache_dict[f'layer_{i}'] = Conv_ReLU.forward(cur_x, self.params[f'W{i}'], self.params[f'b{i}'], conv_param=conv_param)
        cur_x, cache_dict[f'layer_{L}']  = Linear.forward(cur_x, self.params[f'W{L}'], self.params[f'b{L}'])
        scores = cur_x
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        # Replace "pass" statement with your code
        loss, dx = softmax_loss(scores, y)
        dx, grads[f'W{L}'], grads[f'b{L}'] = Linear.backward(dx, cache_dict[f'layer_{L}'])
        for i in range(L-1, -1, -1):
          if i in self.max_pools:
            if self.batchnorm:
              dx, grads[f'W{i}'], grads[f'b{i}'], grads[f'gamma{i}'], grads[f'beta{i}'] = Conv_BatchNorm_ReLU_Pool.backward(dx, cache_dict[f'layer_{i}'])
            else:
              dx, grads[f'W{i}'], grads[f'b{i}'] = Conv_ReLU_Pool.backward(dx, cache_dict[f'layer_{i}'])
          else:
            if self.batchnorm:
              dx, grads[f'W{i}'], grads[f'b{i}'], grads[f'gamma{i}'], grads[f'beta{i}'] = Conv_BatchNorm_ReLU.backward(dx, cache_dict[f'layer_{i}'])
            else:
              dx, grads[f'W{i}'], grads[f'b{i}'] = Conv_ReLU.backward(dx, cache_dict[f'layer_{i}'])
...
class BatchNorm(object):
    ...
    @staticmethod
    def forward(x: torch.Tensor, gamma, beta, bn_param: dict):
        """
        ...
        if mode == 'train':
            ...
            x_var = x.var(correction=0, dim=0)
            # x_var = torch.sum((x - x_mean) ** 2, dim=0) / x.shape[0]
            denom = torch.sqrt(x_var + eps)

            cache['x_mean'], cache['x_var'], cache['denom'] = x_mean, x_var, denom
            cache['x_zero'] = x - x_mean
            cache['x_b'] = (x - x_mean) / denom
    ...
    @staticmethod
    def backward(dout: torch.Tensor, cache: dict):
        """
        ...
        var: torch.Tensor    # D
        mean: torch.Tensor   # D 
        denom: torch.Tensor  # D
        x_zero: torch.Tensor # N, D
        dvar = torch.Tensor
        dmean = torch.Tensor
        x_b = torch.Tensor
        ...
        var, gamma, beta = cache['x_var'], cache['gamma'], cache['beta']
        mean = cache['x_mean']     # D
        denom = cache['denom']   # D
        x_zero = cache['x_zero'] # N, D
        x_b = cache['x_b']       # N, D

        dgamma = torch.sum(dout * x_b, dim=0)
        dbeta  = torch.sum(dout, dim=0)

        dxb    = dout * gamma       # N, D

        dvar   = torch.sum(dxb * x_zero * -0.5 * (denom**(-3)), dim=0)

        dmean  = torch.sum(dxb * -1.0 / denom, dim=0) + dvar * torch.sum(-2.0 * x_zero, dim=0) / (N)  # D  

        dx     = (dxb / denom) + ((dvar * 2.0 * x_zero) / (N)) + (dmean / (N))
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        # Replace "pass" statement with your code
        N, D = dout.shape
        var, denom, x_zero, y, gamma, x_mean = cache['x_var'], cache['denom'], cache['x_zero'], cache['x_b'], cache['gamma'], cache['x_mean']
        dgamma = torch.sum(dout * y, dim=0)
        dbeta  = torch.sum(dout,     dim=0)
        dy     = dout * gamma
        dx     = (N * dy - (x_zero * dy).sum(dim=0) * (denom ** -2) * x_zero - dy.sum(dim=0)) / (N * denom)
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    @staticmethod
    def forward(x: torch.Tensor, gamma, beta, bn_param):
        """
        ...
        y = x.permute(0, 2, 3, 1)
        y = y.reshape(-1, C)
        out, cache = BatchNorm.forward(y, gamma, beta, bn_param=bn_param)
